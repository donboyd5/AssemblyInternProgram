...
# E:\R_projects\projects\AssemblyInternProgram\data

# get the data -------------------------------------------------------------------------
# https://openbudget.ny.gov/spendingForm.html
# https://openbudget.ny.gov/openbudgetdata/SpendingData.xlsx

# DANGER: computer hangs trying to get data directly from the url 
# instead, go here: https://openbudget.ny.gov/spendingForm.html
# find link at bottom of page, and save as...

# this does not work -- oddly, path not found
# url <- "https://openbudget.ny.gov/openbudgetdata/SpendingData.xlsx"
# df1 <- read_excel(url)
...
tab <- tabdata |> 
  ...
  cols_label(namef="Fund group",
             `2023`=html("2022-23<br>(Actuals)"),
             `2024`=html("2023-24<br>(Projected)"),
             pch="% change") |> 
  cols_align(align="left", columns=namef) |> 
  fmt_number(columns=c(`2023`, `2024`, change),
             scale_by = 1e-3,
             decimals = 0) |> 
  fmt_currency(columns=c(`2023`, `2024`, change),
